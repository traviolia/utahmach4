 ...
 *	Added condition_wait_unlock and condition_signal_cthread.
 *
 * $Log: cprocs.c,v $
...
#undef KERNEL
#include <cthreads.h>
#include "mach/cthread_internals.h"
...
#ifdef CTHREAD_SIGNAL
void
condition_wait_unlock(condition_t c, mutex_t m, mutex_t m_unlock)
{
	register cproc_t p = cproc_self();
	...
	mutex_unlock(m_unlock);	/* lock coupling m_unlock -> c->lock */
	mutex_unlock(m);
...
/* 
 * Wake up cthread from condition if found.
 */
void condition_signal_cthread(condition_t c, cthread_t cthread)
{
	cproc_t cproc = (cproc_t) cthread->ur;
	cproc_t p;
	boolean_t found = FALSE;

	spin_lock(&c->lock);
	if ((cproc_t) c->queue.head == cproc) {
		cthread_queue_deq(&c->queue, cproc_t, p);
		found = TRUE;
	} else for (p = (cproc_t) c->queue.head;
		    p != 0;
		    p = p->next)
	{
		if (p->next == cproc) {
			if (cproc->next == 0) {
				c->queue.tail = (cthread_queue_item_t) p;
				p->next = 0;
			} else {
				p->next = cproc->next;
			}
