...
/*
 * HISTORY
 * $Log: e_osf1_sysent.c,v $
 * Revision 1.1.1.2  1995/03/23  01:15:46  law
 * lites-950323 from jvh.
 *
 */
/* 
 *	File:	 emulator/alpha/e_osf1_sysent.c
 *	Authors: Johannes Helander, Helsinki University of Technology, 1995.
 *		 Jukka Virtanen, Helsinki University of Technology, 1995.
 *
 *	System call switch table for alpha osf/1 binaries.
 */
...
int e_osf1_mmap(), e_osf1_stat(), e_osf1_fstat(), e_osf1_lstat();
int e_osf1_getgid(), e_osf1_getuid(), e_osf1_getrlimit(), e_osf1_setrlimit();

int e_machine_fork(), e_machine_vfork(), e_machine_wait();

int e_64_pipe();

struct sysent e_osf1_sysent[] = {
	SYS_NULL(e_indir),		/* 0 */
	sysg(exit, 1),			/* 1 */
	...
	sysg(e_unmount, 2),		/* 22 */
	sysg(e_setuid, 1),		/* 23 */
	sysg(e_osf1_getuid, 0),		/* 24 */
	SYSG(e_exec_with_loader, ?),	/* 25 */
	sysg(e_ptrace, 4),		/* 26 */
	sysg(e_recvmsg, 3),		/* 27 */
	sysg(e_sendmsg, 3),		/* 28 */
	...
	sysg(e_dup, 1),			/* 41 */
	sysg(e_64_pipe, 0),		/* two return values	   42 */
	SYSG(e_set_program_attributes, ?),		/* 43 */
	SYSG(e_profil, ?),		/* 44 profil unimplemented */
	sysg(e_open, 3),		/* 45 */
	sysg(e_sigaction, 3),		/* 46 old obsolete */
	sysg(e_osf1_getgid, 0),		/* 47 */
	...
	sysg(e_osf1_stat, 2),		/* 67 */
	sysg(e_osf1_lstat, 2),		/* 68 */
	sysg(e_sbrk, 1),		/* 69 */
	SYSG(e_sstk, ?),		/* 70 */
	sysg(e_osf1_mmap, 6),		/* 71 */
	SYSG(e_ovadvise, ?),		/* 72 vadvise unimplemented */
	sysg(e_munmap, 2),		/* 73 */
	sysg(e_mprotect, 3),		/* 74 */
	sysg(e_madvise, 3),		/* 75 */
	sysg(e_vhangup, 0),		/* 76 old */
	SYSG(e_kmodcall, ?),		/* 77 */
	sysg(e_mincore, 3),		/* 78 */
	sysg(e_getgroups, 2),		/* 79 */
	sysg(e_setgroups, 2),		/* 80 */
	sysg(e_getpgrp, 0),		/* 81 old */
	SYSG(e_setpgrp, ?),		/* 82 */
	sysg(e_setitimer, 3),		/* 83 */
	SYS_REG(e_machine_wait),	/* 84 old */
	sysg(e_table, 5),		/* 85 */
	...
	sysg(e_nosys, 2),		/* 115 vtrace obsolete */
	...
	sysg(e_rename, 2),		/* 128 */
	sysg(e_bnr_truncate, 2),	/* 129 not old */
	sysg(e_bnr_ftruncate, 2),	/* 130 not old */
	sysg(e_flock, 2),		/* 131 */
	sysg(e_setgid, 1),		/* 132 */
	sysg(e_sendto, 6),		/* 133 */
	...
	sysg(e_osf1_getrlimit, 2),	/* 144 */
	sysg(e_osf1_setrlimit, 2),	/* 145 */
	sysg(e_killpg, 2),		/* 146 old */
	sysg(e_setsid, 0),		/* 147 */
	sysg(e_quotactl, 4),		/* 148 */
	sysg(e_quota, 4),		/* 149 old */
	sysg(e_getsockname, 3),		/* 150 */

	/* OSF1 only */
	NO, NO,
	SYSG(e_pid_block, ?),		/* 153 */
	SYSG(e_pid_unblock, ?),		/* 154 */
	NO,				/* 155 */
	sysg(e_sigaction, 3),		/* 156 */
	SYSG(e_sigwaitprim, ?),		/* 157 */
	sysg(e_nfssvc, 5),		/* 158 */
	...
	NO,				/* 170 */
	NO, NO, NO, NO, NO, NO, NO, NO, NO, NO,	/* 180 */
	SYSG(e_alt_plock, ?),		/* 181 */
	NO, NO,
	SYSG(e_getmnt, ?),		/* 184 */
	NO, NO,
	SYSG(e_alt_sigpending, ?),		/* 187 */
	SYSG(e_alt_setsid, ?),			/* 188 */
	NO, NO,					/* 190 */
	...
	SYSG(e_shmget, ?),			/* 212 */
	SYSG(e_mvalid, ?),			/* 213 */
	SYSG(e_getaddressconf, ?),		/* 214 */
	SYSG(e_msleep, ?),			/* 215 */
	SYSG(e_mwakeup, ?),			/* 216 */
	SYSG(e_msync, ?),			/* 217 */
	SYSG(e_signal, ?),			/* 218 */
	SYSG(e_utc_gettime, ?),			/* 219 */
	SYSG(e_utc_adjtime, ?),			/* 220 */
	NO,
	SYSG(e_security, ?),			/* 222 */
	SYSG(e_kloadcall, ?),			/* 223 */

	NO, NO, NO, NO, NO, NO, NO,		/* 230 */
	NO, NO,
	SYSG(e_getpgid, ?),			/* 233 */
	SYSG(e_setsid, ?),			/* 234 */
	SYSG(e_sigaltstack, ?),			/* 235 */
	SYSG(e_waitid, ?),			/* 236 */
	SYSG(e_priocntlset, ?),			/* 237 */
	SYSG(e_sigsendset, ?),			/* 238 */
	SYSG(e_set_speculative, ?),		/* 239 */
	SYSG(e_msfs_syscall, ?),		/* 240 */
	SYSG(e_sysinfo, ?),			/* 241 */
	SYSG(e_uadmin, ?),			/* 242 */
	SYSG(e_fuser, ?),			/* 243 */
	SYSG(e_proplist_syscall, ?),		/* 244 */
	NO, NO, NO, NO, NO,
	SYSG(e_uswitch, ?),			/* 250 */
	SYSG(e_usleep_thread, ?),		/* 251 */
	SYSG(e_audcntl, ?),			/* 252 */
	SYSG(e_audgen, ?),			/* 253 */
	SYSG(e_sysfs, ?),			/* 254 */
	SYSG(e_subsys_info, ?),			/* 255 */
	SYSG(e_getsysinfo, ?),			/* 256 */
	SYSG(e_setsysinfo, ?),			/* 257 */
	SYSG(e_afs_syscall, ?),			/* 258 */
	SYSG(e_swapctl, ?),			/* 259 */
	SYSG(e_memcntl, ?),			/* 260 */
};

int	e_osf1_nsysent = sizeof(e_osf1_sysent)/sizeof(struct sysent);
